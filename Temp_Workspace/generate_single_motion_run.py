...
def main():
    print("=== Generating Single Motion 2/10: Player_Run (8 Frames) ===")
    
    url = "https://api.pixellab.ai/v2/animate-with-text-v3"
    payload = {
        "first_frame": {"type": "base64", "base64": CONCEPT_B64},
        "action": "fast athletic sprint run cycle with legs cross stride leaning forward",
        "frame_count": 8
    }
    ...
    if r.status_code == 200:
        ...
        print(f"[PixelLab] Single Motion Job Submitted (Player_Run): Job ID = {job_id}")
        ...
        while True:
            ...
            if status == "completed":
                ...
                if pil_images:
                    ...
                    output_path = os.path.join(PLAYER_DIR, "Player_Run.png")
                    sheet.save(output_path, "PNG")
                    print(f"[PixelLab] Successfully saved 2nd single motion: {output_path} ({len(pil_images)} frames, {w}x{h})")
                break
            elif status == "failed":
                ...
                print(f"[PixelLab] Generating Player_Run... ({int(prog * 100)}%)")
                time.sleep(4)
    else:
