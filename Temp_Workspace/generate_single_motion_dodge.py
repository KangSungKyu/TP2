...
def main():
    print("=== Generating Single Motion 5/10: Player_Dodge (8 Frames) ===")
    
    url = "https://api.pixellab.ai/v2/animate-with-text-v3"
    payload = {
        "first_frame": {"type": "base64", "base64": CONCEPT_B64},
        "action": "dashing forward fast low crouch phantom slide",
        "frame_count": 8
    }
    ...
    if r.status_code == 200:
        ...
        print(f"[PixelLab] Single Motion Job Submitted (Player_Dodge): Job ID = {job_id}")
        ...
        while True:
            ...
            if status == "completed":
                ...
                if pil_images:
                    ...
                    print(f"[PixelLab] Successfully saved 5th single motion: {output_path} ({len(pil_images)} frames, {w}x{h})")
                break
            elif status == "failed":
                ...
                print(f"[PixelLab] Generating Player_Dodge... ({int(prog * 100)}%)")
                time.sleep(4)
    else:
