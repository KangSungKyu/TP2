...
def main():
    print("=== Generating Single Motion 4/10: Player_Guard (8 Frames) ===")
    
    url = "https://api.pixellab.ai/v2/animate-with-text-v3"
    payload = {
        "first_frame": {"type": "base64", "base64": CONCEPT_B64},
        "action": "holding katana diagonally in two hands taking solid defensive stance",
        "frame_count": 8
    }
    ...
    if r.status_code == 200:
        ...
        print(f"[PixelLab] Single Motion Job Submitted (Player_Guard): Job ID = {job_id}")
        ...
        while True:
            ...
            if status == "completed":
                ...
                if pil_images:
                    ...
                    print(f"[PixelLab] Successfully saved 4th single motion: {output_path} ({len(pil_images)} frames, {w}x{h})")
                break
            elif status == "failed":
                ...
                print(f"[PixelLab] Generating Player_Guard... ({int(prog * 100)}%)")
                time.sleep(4)
    else:
