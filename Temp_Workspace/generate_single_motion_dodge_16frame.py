...
def main():
    print("=== Generating Nine Sols/Sekiro Refined Motion 5/10: Player_Dodge (16 Frames) ===")
    
    url = "https://api.pixellab.ai/v2/animate-with-text-v3"
    payload = {
        "first_frame": {"type": "base64", "base64": CONCEPT_B64},
        "action": "fast forward ground slide dash, body staying extremely low horizontally, trailing shadow afterimage effect, right leg extended forward left knee bent low",
        "frame_count": 16
    }
    ...
    if r.status_code == 200:
        ...
        print(f"[PixelLab] Job Submitted (16-Frame Player_Dodge): Job ID = {job_id}")
        ...
        while True:
            ...
            if status == "completed":
                ...
                if pil_images:
                    ...
                    output_path = os.path.join(PLAYER_DIR, "Player_Dodge.png")
                    sheet.save(output_path, "PNG")
                    print(f"[PixelLab] Successfully saved 16-frame Player_Dodge: {output_path} ({len(pil_images)} frames, {w}x{h})")
                break
            elif status == "failed":
                ...
                print(f"[PixelLab] Generating 16-frame Player_Dodge... ({int(prog * 100)}%)")
                time.sleep(4)
    else:
