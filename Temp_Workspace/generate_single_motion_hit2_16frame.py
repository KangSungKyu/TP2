...
def main():
    print("=== Generating Nine Sols/Sekiro Refined Motion 8/10: Player_Attack_Hit2 (16 Frames) ===")
    
    url = "https://api.pixellab.ai/v2/animate-with-text-v3"
    payload = {
        "first_frame": {"type": "base64", "base64": CONCEPT_B64},
        "action": "rising diagonal katana slash: twisting torso from low left hip upward across right shoulder in an energetic 45-degree rising arc strike with momentum follow-through",
        "frame_count": 16
    }
    ...
    if r.status_code == 200:
        ...
        print(f"[PixelLab] Job Submitted (16-Frame Player_Attack_Hit2): Job ID = {job_id}")
        ...
        while True:
            ...
            if status == "completed":
                ...
                if pil_images:
                    ...
                    output_path = os.path.join(PLAYER_DIR, "Player_Attack_Hit2.png")
                    sheet.save(output_path, "PNG")
                    print(f"[PixelLab] Successfully saved 16-frame Player_Attack_Hit2: {output_path} ({len(pil_images)} frames, {w}x{h})")
                break
            elif status == "failed":
                ...
                print(f"[PixelLab] Generating 16-frame Player_Attack_Hit2... ({int(prog * 100)}%)")
                time.sleep(4)
    else:
