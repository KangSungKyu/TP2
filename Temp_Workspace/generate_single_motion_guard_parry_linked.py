...
def main():
    print("=== Generating Parry-Linked Single Motion 4/10: Player_Guard (8 Frames) ===")
    
    url = "https://api.pixellab.ai/v2/animate-with-text-v3"
    payload = {
        "first_frame": {"type": "base64", "base64": CONCEPT_B64},
        "action": "holding the deflection stance in low wide crouch with katana held high in two hands taking solid guard block",
        "frame_count": 8
    }
    ...
    if r.status_code == 200:
        ...
        print(f"[PixelLab] Job Submitted (Parry-Linked Player_Guard): Job ID = {job_id}")
        ...
        while True:
            ...
            if status == "completed":
                ...
                if pil_images:
                    ...
                    output_path = os.path.join(PLAYER_DIR, "Player_Guard.png")
                    sheet.save(output_path, "PNG")
                    print(f"[PixelLab] Successfully saved Parry-Linked Player_Guard: {output_path} ({len(pil_images)} frames, {w}x{h})")
                break
            elif status == "failed":
                ...
                print(f"[PixelLab] Generating Parry-Linked Player_Guard... ({int(prog * 100)}%)")
                time.sleep(4)
    else:
