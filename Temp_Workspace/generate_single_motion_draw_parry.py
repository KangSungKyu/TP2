...
def main():
    print("=== Generating Draw Katana & Re-grip Deflect Motion: Player_Parry (8 Frames) ===")
    
    url = "https://api.pixellab.ai/v2/animate-with-text-v3"
    payload = {
        "first_frame": {"type": "base64", "base64": CONCEPT_B64},
        "action": "drawing katana from sheath and re-gripping blade high with both hands to deflect",
        "frame_count": 8
    }
    ...
    if r.status_code == 200:
        ...
        print(f"[PixelLab] Job Submitted (Draw & Re-grip Katana Parry): Job ID = {job_id}")
        ...
        while True:
            ...
            if status == "completed":
                ...
                if pil_images:
                    ...
                    print(f"[PixelLab] Successfully saved Draw & Re-grip Player_Parry: {output_path} ({len(pil_images)} frames, {w}x{h})")
                break
            elif status == "failed":
                ...
                print(f"[PixelLab] Generating Draw & Re-grip Player_Parry... ({int(prog * 100)}%)")
                time.sleep(4)
    else:
