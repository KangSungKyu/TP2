...
def main():
    print("=== Generating Nine Sols Style Single Motion 3/10: Player_Parry (16 Frames) ===")
    
    url = "https://api.pixellab.ai/v2/animate-with-text-v3"
    payload = {
        "first_frame": {"type": "base64", "base64": CONCEPT_B64},
        "action": "low crouch posture sweeping katana upward in sharp crescent arc to deflect attack",
        "frame_count": 16
    }
    ...
    if r.status_code == 200:
        ...
        print(f"[PixelLab] Job Submitted (Nine Sols Player_Parry): Job ID = {job_id}")
        ...
        while True:
            ...
            if status == "completed":
                ...
                if pil_images:
                    ...
                    print(f"[PixelLab] Successfully saved Nine Sols Player_Parry: {output_path} ({len(pil_images)} frames, {w}x{h})")
                break
            elif status == "failed":
                ...
                print(f"[PixelLab] Generating Nine Sols Player_Parry... ({int(prog * 100)}%)")
                time.sleep(4)
    else:
