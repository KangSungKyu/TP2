...
def main():
    print("=== Generating Sekiro-Style Single Motion 3/10: Player_Parry (8 Frames) ===")
    
    url = "https://api.pixellab.ai/v2/animate-with-text-v3"
    payload = {
        "first_frame": {"type": "base64", "base64": CONCEPT_B64},
        "action": "raising katana up high with both hands in deep wide stance to deflect strike",
        "frame_count": 8
    }
    ...
    if r.status_code == 200:
        ...
        print(f"[PixelLab] Job Submitted (Sekiro Player_Parry): Job ID = {job_id}")
        ...
        while True:
            ...
            if status == "completed":
                ...
                if pil_images:
                    ...
                    print(f"[PixelLab] Successfully saved Sekiro Player_Parry: {output_path} ({len(pil_images)} frames, {w}x{h})")
                break
            elif status == "failed":
                ...
                print(f"[PixelLab] Generating Sekiro Player_Parry... ({int(prog * 100)}%)")
                time.sleep(4)
    else:
