...
def main():
    print("=== Generating Nine Sols/Sekiro Refined Motion 6/10: Player_Jump (16 Frames) ===")
    
    url = "https://api.pixellab.ai/v2/animate-with-text-v3"
    payload = {
        "first_frame": {"type": "base64", "base64": CONCEPT_B64},
        "action": "airborne jump trajectory: launching vertically high with legs tucked in midair, transitioning into two-handed overhead katana downward strike arc",
        "frame_count": 16
    }
    ...
    if r.status_code == 200:
        ...
        print(f"[PixelLab] Job Submitted (16-Frame Player_Jump): Job ID = {job_id}")
        ...
        while True:
            ...
            if status == "completed":
                ...
                if pil_images:
                    ...
                    output_path = os.path.join(PLAYER_DIR, "Player_Jump.png")
                    sheet.save(output_path, "PNG")
                    print(f"[PixelLab] Successfully saved 16-frame Player_Jump: {output_path} ({len(pil_images)} frames, {w}x{h})")
                break
            elif status == "failed":
                ...
                print(f"[PixelLab] Generating 16-frame Player_Jump... ({int(prog * 100)}%)")
                time.sleep(4)
    else:
