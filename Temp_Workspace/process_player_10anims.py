...
controller_path = os.path.join(anims_player_dir, "PlayerAnimatorController.controller")

# 1. Collect Player textures (10 Full-Body Animations)
player_textures = glob.glob(os.path.join(player_dir, "*.png"))
print(f"Found {len(player_textures)} Player texture sheets in Assets/Textures/Characters/Player/")
for t in player_textures:
    print(f" - {os.path.basename(t)}")

# 2. Build or Update PlayerAnimatorController YAML for 10 Animations
player_states_config = [
    (1, "Player_Idle"),
    (2, "Player_Run"),
    (3, "Player_Jump"),
    (4, "Player_Parry"),
...
# Collect GUIDs from anim metas if available
clip_guids = {}
for meta_path in glob.glob(os.path.join(anims_player_dir, "Player_*.anim.meta")):
...
with open(controller_path, "w", encoding="utf-8") as f:
    f.write(clean_player_yaml)

print("Successfully updated PlayerAnimatorController.controller for 10 Full-Body Animations!")
