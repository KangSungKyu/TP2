...
def main():
    print("=== Generating Max High-Quality 16-Frame Motion: Player_Parry (API Max Constraint: 16 Frames) ===")
    
    url = "https://api.pixellab.ai/v2/animate-with-text-v3"
    payload = {
        "first_frame": {"type": "base64", "base64": CONCEPT_B64},
        "action": "drawing katana from sheath smoothly, re-gripping handle high with both hands, and holding deep wide stance high vertical blade deflection",
        "frame_count": 16
    }
    ...
    if r.status_code == 200:
        ...
        print(f"[PixelLab] 16-Frame Parry Job Submitted: Job ID = {job_id}")
        ...
        while True:
            ...
            if status == "completed":
                ...
                if pil_images:
                    ...
                    print(f"[PixelLab] Successfully saved 16-frame Player_Parry: {output_path} ({len(pil_images)} frames, {w}x{h})")
                break
            elif status == "failed":
                ...
                print(f"[PixelLab] Generating 16-frame Player_Parry... ({int(prog * 100)}%)")
                time.sleep(4)
    else:
