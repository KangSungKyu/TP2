...
try:
    ...
    font_monster = ImageFont.truetype("arial.ttf", 13)
except Exception:
    font_player = ImageFont.load_default()
    font_garon = ImageFont.load_default()
    font_monster = ImageFont.load_default()

def create_unified_left_dummy_sheet(motion_name, frame_count, width, height, bg_color, border_color, text_color, arrow_color, font, output_path):
    ...
    for i in range(frame_count):
        ...
        # === Unified Facing Left Arrow (FACING LEFT) ===
    ...
    print(f"[Unified Left Generator] Saved: {output_path} ({frame_count} frames, {width}x{height}px, FACING LEFT)")

def main():
    print("=== Unifying All Unit Sprites to FACING LEFT ===")
    ...
    for item in player_items:
        ...
        create_unified_left_dummy_sheet(item["name"], item["frames"], 128, 256, player_bg, player_border, player_text, player_arrow, font_player, out)

    # 2. Garon Boss Dummy Motions (256x512px, Facing LEFT)
    ...
    for item in garon_items:
        ...
        create_unified_left_dummy_sheet(item["name"], item["frames"], 256, 512, garon_bg, garon_border, garon_text, garon_arrow, font_garon, out)

    # 3. Normal Monsters Dummy Motions (Facing LEFT)
    monsters = [
        {"name": "SpearSentry", "width": 154, "height": 307, "bg": (25, 45, 25, 230), "border": (100, 220, 100, 255), "text": (120, 255, 120, 255)},
        {"name": "ShadowStalker", "width": 128, "height": 256, "bg": (35, 25, 45, 230), "border": (180, 100, 220, 255), "text": (200, 120, 255, 255)},
        {"name": "WaveHeavy", "width": 205, "height": 410, "bg": (45, 35, 15, 230), "border": (220, 180, 60, 255), "text": (255, 200, 80, 255)}
    ]
    monster_motions = ["Idle", "Move", "Attack", "Death"]
    
    for m in monsters:
        m_dir = os.path.join(MONSTER_DIR, m["name"])
        os.makedirs(m_dir, exist_ok=True)
        for mot in monster_motions:
            name = f"{m['name']}_{mot}"
            out = os.path.join(m_dir, f"{name}.png")
            create_unified_left_dummy_sheet(name, 8, m["width"], m["height"], m["bg"], m["border"], m["text"], (255, 220, 0, 255), font_monster, out)

    print("\n=== All Units (Player, Garon, Monsters) Unified to FACING LEFT ===")
