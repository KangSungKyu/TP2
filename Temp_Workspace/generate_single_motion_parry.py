...
def main():
    print("=== Generating Single Motion 3/10: Player_Parry (8 Frames) ===")
    
    url = "https://api.pixellab.ai/v2/animate-with-text-v3"
    payload = {
        "first_frame": {"type": "base64", "base64": CONCEPT_B64},
        "action": "raising katana vertically to block incoming attack and sharp counter thrust",
        "frame_count": 8
    }
    ...
    if r.status_code == 200:
        ...
        print(f"[PixelLab] Single Motion Job Submitted (Player_Parry): Job ID = {job_id}")
        ...
        while True:
            ...
            if status == "completed":
                ...
                if pil_images:
                    ...
                    print(f"[PixelLab] Successfully saved 3rd single motion: {output_path} ({len(pil_images)} frames, {w}x{h})")
                break
            elif status == "failed":
                ...
                print(f"[PixelLab] Generating Player_Parry... ({int(prog * 100)}%)")
                time.sleep(4)
    else:
