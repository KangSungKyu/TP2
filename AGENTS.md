...
6. **증분 동기화 및 중복 방지 규칙**: `Conversation ID + turn ID`를 중복 방지 키로 사용한다. 최초 1회 전수 동기화 완료 이후의 정기/자동 동기화 시에는 기존 시트 내용을 유지한 채 **신규 추가된 대화 턴(New Turn)만 증분 추가(Incremental Append, `mode: "append"`)**한다.
